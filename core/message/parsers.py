...
def parse_group_message(event, d):
    ...
    if isinstance(mentions, list):
        ...
        event.is_at_self = any(isinstance(m, dict) and m.get('is_you') is True for m in mentions)
        event.is_at_other_bot = any(isinstance(m, dict) and m.get('bot') is True and m.get('is_you') is not True for m in mentions)
        event.is_at_all = any(isinstance(m, dict) and m.get('scope') == 'all' for m in mentions)
