...
def _build_dau_message(event, stats, date, elapsed_ms, y_stats=None, is_today=False):
    """构建 DAU 统计消息"""
    time_suffix = (
        f" (截至{datetime.now().hour:02d}:{datetime.now().minute:02d})"
        if is_today
        else ""
    )
    info = [
...
@handler(
    r"^用户统计$", name="用户统计", desc="查看当前机器人的用户/群统计", owner_only=True
)
async def get_stats(event, match):
    ...
    group_counts = [
        (g["group_id"], _count_json_array(g.get("users"))) for g in (all_groups or [])
    ]
...
async def _handle_today_dau(event, bot):
    ...
    elapsed = round((time.time() - t0) * 1000)
    msg = _build_dau_message(
        event, stats, datetime.now(), elapsed, y_stats=y_stats, is_today=True
    )
